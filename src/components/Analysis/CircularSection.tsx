...
export const CircularSection = ({ activeTab, results, inputs, setInputs, generateReport, mergedMomentData, downloadCSV }: any) => {
  if (activeTab === 'summary') {
    return (
      <div className="flex flex-col gap-8 pb-16">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h3 className="text-xl font-bold text-slate-800">Circular Section Summary</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <StatCard 
            ...
            value={Math.max(...(results.mcStrong?.moments || [0])).toFixed(0)} 
            unit="kN-m"
            subLabel="Max capacity at failure"
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8 items-stretch h-full">
           <div className="md:col-span-2 h-full min-h-[350px]">
              <CrossSectionVisualizer sectionType="circular" inputs={inputs} isPrintMode={false} />
           </div>
           <div className="md:col-span-3 h-full min-h-[450px] border border-slate-100 rounded-xl p-5 bg-slate-50 flex flex-col">
              ...
              <div className="flex-grow">
                <InteractiveLineChart data={mergedMomentData} xKey="curvature" yKey="moment" xLabel="Curvature (1/m)" yLabel="Moment (kN-m)" className="h-full" />
              </div>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pb-16">
        <div className="h-[550px] border border-slate-100 rounded-2xl p-6 shadow-sm md:col-span-2 relative">
           <div className="absolute top-6 right-6 z-10">
              <button 
                onClick={() => downloadCSV(mergedMomentData, 'moment_curvature_circular.csv', ['curvature', 'moment', 'strainSteel', 'strainConc'])}
           ...
           <InteractiveLineChart data={mergedMomentData} xKey="curvature" yKey="moment" title="Moment vs. Curvature (Circular)" xLabel="Curvature (1/m)" yLabel="Moment (kN-m)"/>
        </div>
        <div className="h-[450px] border border-slate-100 rounded-2xl p-6 shadow-sm">
          <InteractiveLineChart data={mergedMomentData} xKey="curvature" yKey="strainSteel" title="Steel Strain vs. Curvature" color="#ef4444" xLabel="Curvature (1/m)" yLabel="Tensile Strain (%)"/>
        </div>
        <div className="h-[450px] border border-slate-100 rounded-2xl p-6 shadow-sm">
          <InteractiveLineChart data={mergedMomentData} xKey="curvature" yKey="strainConc" title="Concrete Strain vs. Curvature" color="#10b981" xLabel="Curvature (1/m)" yLabel="Comp. Strain (%)"/>
        </div>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 gap-8 pb-16">
        <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex-grow">
            ...
            <p className="text-xs text-slate-500">Adjust parameters for circular rotation calculations</p>
          </div>
        ...
        <div className="h-[600px] border border-slate-100 rounded-2xl p-6 shadow-sm relative">
           <div className="absolute top-6 right-6 z-10">
              <button 
                onClick={() => downloadCSV(mergedMomentData, 'moment_rotation_circular.csv', ['rotation', 'moment'])}
                className="flex items-center gap-2 px-3 py-1.5 bg-white text-slate-700 rounded-lg text-xs font-bold hover:bg-slate-50 transition-colors border border-slate-200 shadow-sm"
              >
                <Download size={14} /> Download CSV
              </button>
           </div>
           <InteractiveLineChart data={mergedMomentData} xKey="rotation" yKey="moment" title="Moment vs. Rotation (Circular)" xLabel="Rotation (rad)" yLabel="Moment (kN-m)"/>
        </div>
